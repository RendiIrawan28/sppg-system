...
<div class="flex flex-col md:flex-row gap-6 mb-8">
    <div class="bg-white p-6 rounded-xl shadow-lg flex-1">
        ...
        <div class="flex items-center justify-between">
            ...
            <span class="text-sm text-gray-500">
                Mode saat ini digunakan oleh perangkat RFID.
            </span>
        </div>

        <p class="mt-2 text-sm text-gray-500">
            @if($modeStatus == 'registrasi')
            Mode REGISTRASI : Setiap kartu yang di-scan akan didaftarkan ke Pegawai yang belum memiliki UID.
            @else
            Mode PRESENSI : Kartu yang di-scan akan mencatat jam masuk/keluar.
            @endif
        </p>
        ...
        <div class="mt-6 flex space-x-3">
            ...
            <form action="{{ route('presensi.mode.registrasi') }}" method="POST">
                @csrf
                <button type="submit" class="w-full bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-150 ease-in-out">
            ...
            <form action="{{ route('presensi.mode.presensi') }}" method="POST">
                @csrf
                <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition duration-150 ease-in-out">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 flex-2">
        <div class="bg-white p-5 rounded-xl shadow-lg border-l-4 border-indigo-500">
            <p class="text-sm font-medium text-gray-500">Total Relawan</p>
...
<div class="bg-white p-6 rounded-xl shadow-lg">
    <h2 class="text-2xl font-semibold text-gray-800 mb-6 flex items-center">
        Log Presensi Hari Ini ({{ \Carbon\Carbon::today()->translatedFormat('l, d F Y') }})
        <span id="update-status" class="ml-4 text-xs font-medium px-2 py-1 rounded-full bg-blue-100 text-blue-600">Memuat...</span>
    </h2>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Lembur (Menit)</th>
...
<script>
    ...
    function fetchData() {
        ...
        updateStatus.className = 'ml-4 text-xs font-medium px-2 py-1 rounded-full bg-yellow-100 text-yellow-600';
            ...
            .then(data => {
                ...
                if (data.length === 0) {
                    ...
                    data.forEach(item => {
                        ...
                        const rowClass = item.jam_keluar === '—' ? 'bg-yellow-50' : '';
                        if (item.jam_keluar === '—') countBelumKeluar++;

                        const jamKeluarDisplay = item.jam_keluar === '—' ?
                        ...
                        row.innerHTML = `
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${item.telat ?? '-'}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${item.lembur ?? '-'}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${item.total_jam === '—' ? '-' : item.total_jam + ' Menit'}</td>
                            `;
                ...
                updateStatus.className = 'ml-4 text-xs font-medium px-2 py-1 rounded-full bg-green-100 text-green-600';
            })
            .catch(error => {
                console.error("Fetch error:", error);
                updateStatus.textContent = 'Gagal (Cek Konsol)';
                updateStatus.className = 'ml-4 text-xs font-medium px-2 py-1 rounded-full bg-red-100 text-red-600';
            });
    }
