            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            @if (! $riwayat->jam_keluar || $status === 'open')
                            <form action="{{ route('presensi.manual.checkout', $riwayat) }}"
                                method="POST"
                                class="space-y-2"
                                onsubmit="return confirm('Yakin checkout manual pegawai ini?');">
                                @csrf

                                <input type="datetime-local"
                                    name="jam_keluar"
                                    value="{{ now('Asia/Jakarta')->format('Y-m-d\TH:i') }}"
                                    class="w-48 rounded-lg border-gray-300 shadow-sm p-2 border text-xs">

                                <input type="text"
                                    name="catatan"
                                    placeholder="Catatan"
                                    value="Kartu/hardware bermasalah"
                                    class="w-48 rounded-lg border-gray-300 shadow-sm p-2 border text-xs">

                                <button type="submit"
                                    class="w-48 bg-orange-600 hover:bg-orange-700 text-white font-semibold py-2 px-3 rounded-lg text-xs">
                                    Checkout Manual
                                </button>
                            </form>
                            @else
