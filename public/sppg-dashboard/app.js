...
let appState = {
    currentTab: "overview",
    apiUrl: localStorage.getItem("sppg_api_url") || "/api/dashboard",
    data: {},
...
function normalizeDashboardData(data) {
    ...
    return {
        ...
        gudang_stok: Array.isArray(d.gudang_stok) ? d.gudang_stok : [],

        gudang_bahan_masuk: Array.isArray(d.gudang_bahan_masuk)
            ? d.gudang_bahan_masuk
            : [],

        gudang_bahan_keluar: Array.isArray(d.gudang_bahan_keluar)
            ? d.gudang_bahan_keluar
            : [],
    };
}
...
function getAslapRecipientTotals(list, includeTendik = false) {
    const grouped = {};

    list.forEach((item) => {
        const nama = String(item["Nama Penerima"] || "Tanpa Nama").trim();

        const porsiBesar = toNumber(item["Porsi Besar"]);
        const porsiKecil = toNumber(item["Porsi Kecil"]);
        const tendik = includeTendik ? toNumber(item["Tendik"]) : 0;

        const rawTotal = toNumber(item["Total"]);

        const total =
            rawTotal > 0 ? rawTotal : porsiBesar + porsiKecil + tendik;
        ...
        grouped[nama].porsiBesar += porsiBesar;
        grouped[nama].porsiKecil += porsiKecil;
        grouped[nama].tendik += tendik;
        grouped[nama].total += total;
    });
...
function getCategoryLabel(key) {
    const labels = {
        ...
        gudang_bahan_masuk: "Gudang Bahan Masuk",
        gudang_bahan_keluar: "Gudang Bahan Keluar",
    };
...
function initModal() {
    ...
    if (DOM.instructionsToggle) {
        DOM.instructionsToggle.addEventListener("click", () => {
            ...
            DOM.instructionsToggle.setAttribute(
                "aria-expanded",
                String(isActive),
            );

            if (DOM.instructionsContent) {
                DOM.instructionsContent.style.display = isActive
                    ? "block"
                    : "none";
            }
        });
...
async function fetchData() {
    ...
    try {
        const targetUrl = createUrlWithDate(
            appState.apiUrl,
...
function updateHeader() {
    const titles = {
        ...
        gudang_stok: {
            t: "Stok Gudang",
            d: `Monitoring stok bahan gudang tanggal ${appState.selectedDate}.`,
        },

        gudang_bahan_masuk: {
            t: "Bahan Masuk Gudang",
            d: `Riwayat penerimaan bahan gudang tanggal ${appState.selectedDate}.`,
        },

        gudang_bahan_keluar: {
            t: "Bahan Keluar Gudang",
            d: `Riwayat pengeluaran bahan gudang tanggal ${appState.selectedDate}.`,
        },
    };
...
function renderKpiCards() {
    ...
    const aslapDistribusiTotal = getDataList("aslap_distribusi").length;

    const aslapPlanningTotal = getDataList("aslap_planning").length;

    const gudangStokTotal = getDataList("gudang_stok").length;
    const gudangMasukTotal = getDataList("gudang_bahan_masuk").length;
    const gudangKeluarTotal = getDataList("gudang_bahan_keluar").length;

    const gudangPerluPerhatian = getDataList("gudang_stok").filter((item) => {
        const status = String(
            item["Status"] || item["status"] || "",
        ).toLowerCase();
        return status.includes("menipis") || status.includes("habis");
    }).length;
    ...
    distribusiList.forEach((item) => {
        ...
        if (jumlahPorsi > 0) {
            ...
            totalPorsiDistribusi +=
                toNumber(item["Porsi Besar"]) + toNumber(item["Porsi Kecil"]);
        }
    });
    ...
    if (appState.currentTab === "overview") {
      ...
      ${metricCard(
          "info",
          "Gudang",
          gudangMasukTotal + gudangKeluarTotal,
          `${gudangStokTotal} jenis bahan`,
          "fa-warehouse",
      )}

      ${metricCard(
          gudangPerluPerhatian > 0 ? "danger" : "success",
          "Stok Perhatian",
          gudangPerluPerhatian,
          "Stok menipis / habis",
    ...
    if (appState.currentTab === "distribusi") {
        ...
        list.forEach((item) => {
            ...
            totalPorsi += jumlah > 0 ? jumlah : besar + kecil;
    ...
    if (appState.currentTab === "aslap_planning") {
        ...
        list.forEach((item) => {
            ...
            grandTotal += totalItem > 0 ? totalItem : besar + kecil;
        });
    ...
    // GUDANG STOK
    // ==========================================================
    if (appState.currentTab === "gudang_stok") {
        let aman = 0;
        let menipis = 0;
        let habis = 0;
        ...
        list.forEach((item) => {
            ...
            if (status.includes("habis")) {
                habis++;
            } else if (status.includes("menipis")) {
                menipis++;
            } else {
                aman++;
            }
        ...
        html = `
    ${metricCard("info", "Jenis Bahan", total, "Total bahan tercatat", "fa-boxes-stacked")}
    ${metricCard("success", "Aman", aman, "Stok aman", "fa-circle-check")}
    ${metricCard("warning", "Menipis", menipis, "Perlu perhatian", "fa-triangle-exclamation")}
    ${metricCard("danger", "Habis", habis, "Perlu pengadaan", "fa-circle-xmark")}
  `;
    ...
    // GUDANG BAHAN MASUK
    // ==========================================================
    if (appState.currentTab === "gudang_bahan_masuk") {
        let totalJumlah = 0;

        list.forEach((item) => {
            totalJumlah += toNumber(item["Jumlah"] || item["jumlah"]);
        });

        html = `
    ${metricCard("info", "Total Entri", total, "Transaksi bahan masuk", "fa-arrow-down")}
    ${metricCard("success", "Total Jumlah", totalJumlah.toFixed(2), "Akumulasi bahan masuk", "fa-scale-balanced")}
    ${metricCard("warning", "Supplier", new Set(list.map((i) => i["Nama Supplier"] || i["nama_supplier"] || "-")).size, "Jumlah supplier", "fa-truck-field")}
    ${metricCard("info", "Status", total > 0 ? "Ada Data" : "-", "", "fa-circle-info")}
  `;
    ...
    // GUDANG BAHAN KELUAR
    // ==========================================================
    if (appState.currentTab === "gudang_bahan_keluar") {
        let totalJumlah = 0;
        ...
        html = `
    ${metricCard("info", "Total Entri", total, "Transaksi bahan keluar", "fa-arrow-up")}
    ${metricCard("danger", "Total Jumlah", totalJumlah.toFixed(2), "Akumulasi bahan keluar", "fa-scale-balanced")}
    ${metricCard("warning", "Divisi Penerima", new Set(list.map((i) => i["Divisi Penerima"] || i["divisi_penerima"] || "-")).size, "Tujuan pengeluaran", "fa-people-carry-box")}
    ${metricCard("info", "Status", total > 0 ? "Ada Data" : "-", "", "fa-circle-info")}
  `;
...
function renderChart() {
    ...
    if (appState.currentTab === "overview") {
        ...
        values = [
            ...
            getDataList("gudang_bahan_masuk").length +
                getDataList("gudang_bahan_keluar").length,
        ];
        ...
        const isDistribusi = appState.currentTab === "aslap_distribusi";

        const list = getDataList(appState.currentTab);

        const recipientTotals = getAslapRecipientTotals(list, isDistribusi);

        labels = recipientTotals.map((item) => item.nama);

        values = recipientTotals.map((item) => item.total);

        title = isDistribusi
            ? "Total Distribusi per Penerima"
            : "Total Planning per Penerima";

        datasetLabel = isDistribusi ? "Total Distribusi" : "Total Planning";

        type = "bar";
        horizontal = true;

        if (DOM.chartTypeIndicator) {
            DOM.chartTypeIndicator.textContent = "Jumlah Porsi";
        }
        ...
        if (chartWrapper) {
            ...
            chartWrapper.style.height = `${Math.max(
                minimumHeight,
                recipientTotals.length * heightPerItem,
            )}px`;
        }
    } else {
...
function renderSummaryList() {
    ...
    if (appState.currentTab === "overview") {
        if (
            ...
            const recipientTotals = getAslapRecipientTotals(
                getDataList(appState.currentTab),
                isDistribusi,
            );
    ...
    `;
            ...
            const displayedItems = recipientTotals.slice(0, 8);
    ...
    `,
        ...
        const rows = [
            ...
            ["Stok Gudang", getDataList("gudang_stok").length],
        ];
        ...
        Object.keys(appState.data).forEach((key) => {
            ...
            arr.slice(-3).forEach((item) => {
                list.push({
                    ...
                    petugas:
                        item["Nama Petugas"] || item["Nama Penerima"] || "-",
                });
            });
        });
    } else {
...
function getColumnsForTab(tab) {
    ...
    if (tab === "gudang_stok") {
        return [
            { k: "Nama Bahan", l: "Nama Bahan" },
            { k: "Satuan", l: "Satuan" },
            { k: "Total Masuk", l: "Total Masuk" },
            { k: "Total Keluar", l: "Total Keluar" },
            { k: "Stok", l: "Sisa Stok" },
            { k: "Status", l: "Status" },
        ];
    }

    if (tab === "gudang_bahan_masuk") {
        return [
            { k: "ID", l: "ID" },
            { k: "Tanggal", l: "Tanggal" },
            { k: "Nama Petugas", l: "Petugas" },
            { k: "Nama Supplier", l: "Supplier" },
            { k: "Nama Bahan", l: "Nama Bahan" },
            { k: "Jumlah", l: "Jumlah" },
            { k: "Satuan", l: "Satuan" },
            { k: "Kondisi Bahan", l: "Kondisi" },
            { k: "Tanggal Kedaluwarsa", l: "Kedaluwarsa" },
            { k: "Catatan", l: "Catatan" },
            { k: "Foto URL", l: "Foto" },
        ];
    }

    if (tab === "gudang_bahan_keluar") {
        return [
            { k: "ID", l: "ID" },
            { k: "Tanggal", l: "Tanggal" },
            { k: "Nama Petugas", l: "Petugas" },
            { k: "Nama Penerima", l: "Penerima" },
            { k: "Divisi Penerima", l: "Divisi" },
            { k: "Nama Bahan", l: "Nama Bahan" },
            { k: "Jumlah", l: "Jumlah" },
            { k: "Satuan", l: "Satuan" },
            { k: "Catatan", l: "Catatan" },
