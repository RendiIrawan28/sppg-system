...
class SppgReportService
{
    ...
    public function toMobileItem(string $category, array $row): array
    {
        ...
        return match ($category) {
            ...
            'persiapan_bahan' => $base + [
                'namaBahan' => (string) ($row['Nama Bahan'] ?? ''),
                'banyaknya' => (string) ($row['Banyaknya'] ?? 0),
                'satuan' => (string) ($row['Satuan'] ?? ''),
                'baik' => (string) ($row['Baik'] ?? 0),
                'rusak' => (string) ($row['Rusak'] ?? 0),
                'sedang' => (string) ($row['Sedang'] ?? 0),
            ...
            'pengolahan_suhu' => $base + [
                'waktu' => (string) ($row['Waktu'] ?? ''),
                'namaProduk' => (string) ($row['Nama Produk'] ?? ''),
                'suhuProduk' => (string) ($row['Suhu Produk'] ?? 0),
                'namaPetugas' => (string) ($row['Nama Petugas'] ?? ''),
                'paraf' => (string) ($row['Paraf'] ?? ''),
                'fotoUrl' => (string) ($row['Foto URL'] ?? ''),
            ],
            'pengolahan_produksi' => $base + [
                'menu' => (string) ($row['Menu'] ?? ''),
                'bahanBaku' => (string) ($row['Bahan Baku'] ?? ''),
                'qty' => (string) ($row['Qty'] ?? 0),
                'satuanBahan' => (string) ($row['Satuan Bahan'] ?? ''),
                'waktuProduksi' => (string) ($row['Waktu Produksi'] ?? ''),
                'jamMulai' => (string) ($row['Jam Mulai'] ?? ''),
                'hasilAkhir' => (string) ($row['Hasil Akhir'] ?? 0),
                'satuanHasil' => (string) ($row['Satuan Hasil'] ?? ''),
                'namaPetugas' => (string) ($row['Nama Petugas'] ?? ''),
                'fotoUrl' => (string) ($row['Foto URL'] ?? ''),
            ],
            'pemorsian_ompreng' => $base + [
                'rute' => (string) ($row['Rute'] ?? ''),
                'qtyOmprengKecil' => (string) ($row['Qty Ompreng Kecil'] ?? 0),
                'qtyOmprengBesar' => (string) ($row['Qty Ompreng Besar'] ?? 0),
                'waktuPemorsian' => (string) ($row['Waktu Pemorsian'] ?? ''),
                'namaPetugas' => (string) ($row['Nama Petugas'] ?? ''),
                'fotoUrl' => (string) ($row['Foto URL'] ?? ''),
            ],
            'pemorsian_sisa' => $base + [
                'rute' => (string) ($row['Rute'] ?? ''),
                'waktuCekSisa' => (string) ($row['Waktu Cek Sisa'] ?? ''),
                'jenisMakanan' => (string) ($row['Jenis Makanan'] ?? ''),
                'beratSisaKg' => (string) ($row['Berat Sisa Kg'] ?? 0),
                'keterangan' => (string) ($row['Keterangan'] ?? ''),
                'namaPetugas' => (string) ($row['Nama Petugas'] ?? ''),
                'fotoUrl' => (string) ($row['Foto URL'] ?? ''),
            ],
